...
repeated tool output. We call it **in-process** on the context blocks CLIC
...
long-running process: scope is exactly the prompts CLIC assembles — external
...
- Headroom's router **protects user-role messages verbatim** and only crushes
  ...
  as the relevance query) and the bulky context as the tool message. CLIC's
  instructions and the CLIC_RESULT_V1 contract can never be rewritten.
...
async def compress_context(text: str, instructions: str = "") -> str:
    """Compress one bulky, already-redacted context block for a prompt CLIC is
