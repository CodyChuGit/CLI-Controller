...
def orchestrator_consult_prompt(usage: dict, task_state: str, trigger: str, output_tail: str) -> str:
    ...
    return (
        f"{budget_context_header(usage)}\n\n"
        "You are the controller model for Command Line Interface Controller "
        "(CLI Controller IDE), supervising a task whose step just "
...
def orchestrator_chat_prompt(usage: dict, workspace_summary: str, transcript: str, message: str) -> str:
    ...
    return (
        f"{budget_context_header(usage)}\n\n"
        "You are the controller model for Command Line Interface Controller "
        "(CLI Controller IDE), a local cockpit that routes coding work "
        ...
        "Be compact and concrete — prefer specific next actions in CLIC (create a task, run a step "
        ...
        + "You can create CLIC tasks AND queue work to the agents — the system executes the queue "
        "automatically, cueing one step per agent at a time, in order. Default to starting a task with "
        "`codex_spec` (the planning step) and queueing only that first step; CLIC reports back after it "
...
def direct_chat_prompt(provider: str, transcript: str, message: str) -> str:
    ...
    convo = f"Conversation so far:\n{transcript}\n\n" if transcript else ""
    return (
        f"You are `{provider}` in a direct chat inside Command Line Interface Controller "
        "(CLI Controller IDE). The user is talking to you "
