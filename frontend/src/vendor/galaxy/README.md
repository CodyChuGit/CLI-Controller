...
CLIC renders the upstream three.js `GraphScene` galaxy **natively** (fed by the
...
They are excluded from CLIC's eslint/prettier (see `.prettierignore` and
`eslint.config.js`) so they stay byte-for-byte upstream. All CLIC-side wiring
