...
- Add TestApp Calendar Scheduler handoff records for overflow work when user
  ...
  execution.
- Keep overflow tasks in CLIC's queue/task model and resume them through normal
...
Goal: turn the right-hand chat dock and Tasks tab into native CLIC feature
...
- The Agent Dock gives Codex, Claude Code, Antigravity, and controller workflows
  a live VS Code-style panel experience inside CLIC.
...
Goal: give CLIC an independent app-window launch path without Electron, Tauri,
native desktop packaging, deprecated Chrome Apps, or a Chrome Extension shell.

Tasks:

- Add a PWA manifest with CLI Controller IDE naming, tagline, bean icons,
...
- Add an optional `scripts/create-macos-app-mode.sh` that generates a thin
  `CLI Controller IDE.app` wrapper using the bean icon and delegating to the
  launcher script.
- Keep the normal browser URL flow working exactly as it does today.

Acceptance criteria:

- Chrome can install or open CLIC as a standalone PWA-style window.
