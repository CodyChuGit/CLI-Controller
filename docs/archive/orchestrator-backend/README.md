...
Interface Traffic Controller (CLI Controller IDE).
...
- [Streaming Renderer Decision](../streaming-renderer-decision.md) records the
  renderer/package decision: keep one CLIC event stream and use an internal
...
- Keep CLIC local-first. The backend shells out to user-owned CLIs and stores
...
- Make automatic execution explainable. Every queued step, skipped step, direct
  ...
  event.
- Build the right-hand Agent Dock and Tasks tab as CLIC-native VS Code-style
...
- Treat TestApp Calendar Scheduler as an overflow handoff for quota/user-limit
  delays. Scheduled work must return through normal CLIC queue, policy, and
  approval flows.
