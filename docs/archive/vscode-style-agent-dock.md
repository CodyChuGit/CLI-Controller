...
Command Line Interface Traffic Controller (CLI Controller IDE) should mimic the
...
CLIC's local-first backend, queue, approvals, task files, and CLI execution
model.

This is feasible because CLIC already owns the required primitives:
...
are native CLIC product surfaces, not wrappers around VS Code.
...
an active controller, Codex, Claude Code, or Antigravity run, CLIC should show
...
- Live generated text should use the internal `SmoothStreamingText` renderer over
  CLIC's shared event store so the dock feels like a real CLI without opening a
...
- Command palette actions scoped to CLIC-native actions only, such as run
