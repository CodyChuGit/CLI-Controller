...
Command Line Interface Traffic Controller (CLI Controller IDE) should support
...
This gives CLIC the independent-window feel of apps like VJbooth while keeping
...
CLIC should feel launchable as a local app, but remain a local-first web app
...
- User launches **CLI Controller IDE** from a script or macOS app icon.
...
- The app uses the normal CLIC local URL and existing backend APIs.
...
- Write backend stdout/stderr to a local log file, for example
  `.agentflow/runtime/app-mode-backend.log` or `/tmp/clic-controller/backend.log`.
...
- `--user-data-dir=<local-runtime-profile>` for a separate CLIC Chrome profile.
...
- Generates `dist/CLI Controller IDE.app`.
- Uses the bean icon.
...
- `manifest.webmanifest` is served from `http://localhost:8787` and points to
  separate no-alpha `any` and safe-zone `maskable` CLIC bean icons.
- Chrome can install or run CLIC as a standalone PWA-style app window.
