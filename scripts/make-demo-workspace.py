...
(DEMO / "package.json").write_text(json.dumps({
    "name": "orbit-dashboard", "version": "2.4.1", "private": True,
...
task = {
    ...
    "steps": {
        "codex_spec": {
            "status": "succeeded", "provider": "codex", "runId": "a71f0c93be22",
            "updatedAt": t("09:44", "18"), "exitCode": 0,
        ...
        "claude_implement": {
            "status": "succeeded", "provider": "claude", "runId": "5c2e8d10a447",
            "updatedAt": t("09:51", "36"), "exitCode": 0,
        ...
        "gemini_qa": {
            "status": "succeeded", "provider": "antigravity", "runId": "9b4a17ee0c85",
            "updatedAt": t("09:55", "02"), "exitCode": 0,
            "artifactsWritten": ["05_QA_RESULTS.md"], "codeChanged": [],
            "promptFile": f"{D.replace('-','')}-095136-gemini_qa.prompt.txt",
            "logFile": f"{D.replace('-','')}-095136-gemini_qa.log",
        },
        "codex_review": {
            "status": "succeeded", "provider": "codex", "runId": "3f8d55c1907b",
            "updatedAt": t("09:57", "44"), "exitCode": 0,
            "artifactsWritten": ["07_CODEX_FINAL_REVIEW.md"], "codeChanged": [],
...
REPLIES = {
    "094120-codex_spec": ("codex exec --model gpt-5.6-sol", """Wrote the two handoff docs:
...
No production code touched."""),
    "094432-claude_implement": ("claude -p --permission-mode acceptEdits --model claude-opus-5", """Done. Implemented the plan across 4 files:
...
Tests added next to both routes, including the replayed-event case the spec asked for."""),
    "095136-gemini_qa": ("agy --sandbox -p", """`npm test` → 18 passed, 0 failed.
...
No blocking issues. Wrote `05_QA_RESULTS.md`."""),
    "095502-codex_review": ("codex exec --model gpt-5.6-sol", """Reviewed the diff against the spec.
...
def q(qid, step, label, provider, start, end):
    return {"id": qid, "taskId": TASK_ID, "step": step, "label": label, "provider": provider,
            "status": "done", "source": "orchestrator", "enqueuedAt": t(start), "note": None,
            "runId": qid + "aa11", "attempt": 1, "providerOverride": None,
            "startedAt": t(start), "finishedAt": t(end)}
(AF / "queue.json").write_text(json.dumps({"items": [
...
messages = [
    ...
    {"role": "assistant", "provider": "claude", "durationMs": 11240, "time": t("09:41"),
...
(AF / "chat.json").write_text(json.dumps(
    {"messages": messages, "updatedAt": t("09:57", "51"), "channels": {}}, indent=2) + "\n")

(AF / "config.json").write_text(json.dumps({
